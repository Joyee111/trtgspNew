...
<div>
   <font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;停售管理&nbsp;>&nbsp;停售审核</font>
  	<div class="tab_biao">
	...
	<div class="tab_con">
		<div class="caozuo_box">
  		<form action="${ctx}/drugState/stopcell/dshlist.html" method="post">
  			<input type="hidden" name="ptmeth" value="${ptmeth}"/>
			<input type="hidden" name="tz" value="${thispage}"/>
				通用名称：<input name="ypname" type="text" value="${ypname}" class="text1">
			<shiro:hasPermission name="StopSale_auditing_find">
...
<form action="dshlist.html" id="audit_syqydsh">
	...
	<display:table name="recordlist" cellspacing="0" cellpadding="0" requestURI=""  style="width:98%;" id="records" pagesize="${pageSize}" class="table" export="true" size="${resultSize}" partialList="true">
		...
		<shiro:hasPermission name="StopSale_auditing_look">
...
<c:if test="${deleteItemFlag  eq 'true'}">
...
<input type="button" onclick="batchAction('1')" value="批量驳回" class="btn_big"/>
	...
	function batchAction(type){
		...
		from.action="${ctx}/drugState/stopcell/audit.html";
		if(type=="0"){
