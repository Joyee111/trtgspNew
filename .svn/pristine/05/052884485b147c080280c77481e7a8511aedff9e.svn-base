...
<script type="text/javascript">
	function onDelete(){
		...
		if(r){
			from.action="${ctx}/drugState/checkreturn/dlrdel.html";
			from.submit();
		}else{
			return;
		}
    ...
    function add(){
    	...
    	form.action="${ctx}/drugState/checkreturn/dlradd.html";
    	form.submit();
    }
	function deletess(a){
...
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;退货复检&nbsp;>&nbsp;复检</font>
	<div class="tab_biao">
	<!-- <shiro:hasPermission name="ReturnCheckAccept_recorded">
		...
		<shiro:hasPermission name="ReturnCheckAccept_auditing">
			<a href="rechecklist.html"  class="on"><span><font>复检</font>
		</span>
		</a>
		</shiro:hasPermission>
...
<div class="tab_con">
	<div class="caozuo_box">
		<form action="${ctx}/drugState/checkreturn/rechecklist.html" method="post" >
			...
			<shiro:hasPermission name="ReturnCheckAccept_recorded_find">
...
<form action="" id="recordsss">
	...
	<display:table name="recordlist" cellspacing="0" cellpadding="0" requestURI=""  style="width:98%;" id="records" pagesize="${pageSize}" class="table" export="true" size="${resultSize}" partialList="true">
		...
		<display:column escapeXml="true" sortable="true" titleKey="退回数量" >
		</display:column>
		...
		<img src="../../images/edit.gif"/><a href="${ctx}/drugState/checkreturn/recheckview.html?id=<c:out value="${records.id}"/>&thispage=${thispage}">
