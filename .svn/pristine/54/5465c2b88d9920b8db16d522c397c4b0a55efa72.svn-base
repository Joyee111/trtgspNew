...
<body>
			<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">企业资质管理&nbsp;>&nbsp;购货单位&nbsp;>&nbsp;购货单位审核</font>
			<div class="tab_biao">
        	<shiro:hasPermission name="PurchaseUnit_recorded">
				<a href="ghdwdlr.html" ><span><font>录入</font>
				</span>
				</a>
			</shiro:hasPermission>
			<shiro:hasPermission name="PurchaseUnit_auditing">
				<a href="ghdwdsh.html" class="on"><span><font>审核</font>
				</span>
				</a>
			</shiro:hasPermission>
        ...
        <div class="tab_con">
        	...
        	<form name="query_ghdwdsh" id="query_ghdwdsh" action="query_ghdwdsh.html" method="post">
                ...
                <shiro:hasPermission name="PurchaseUnit_auditing_find">
                <input name="" type="submit" class="btn_big" value="查询"/>
                </shiro:hasPermission>
              </form>
            </div>
          <form name="audit_ghdwdsh" action="batchOperate_dhdwdsh.html" id="audit_ghdwdsh" method="post" >
 			<display:table name="purchaseUnitsWaitList" cellspacing="0" cellpadding="0" requestURI="" style="width:98%;" id="purchaseUnit" pagesize="${pagesize}" class="table" export="true" size="${resultSize}" partialList="true">
 				<display:column title="<input type='checkbox' id='checkall' name='checkall' value='${id }' onclick='checkAll(\"operate_id\")' />全选" media="html"><input type="checkbox" name="operate_id" value="${purchaseUnit.id}" ></display:column>
				...
				<display:column  escapeXml="true" sortable="false" titleKey="开户公司">
				 ...
				 <shiro:hasPermission name="PurchaseUnit_auditing_look">
				<display:column titleKey="操作" autolink="true" media="html" class="border_rnone">
				<img src="../images/look.gif"/><a href="view_ghdwdsh.html?id=${purchaseUnit.id}">查看</a>
				</display:column>
				</shiro:hasPermission>
	...
	function batchAction(type){
		var from =document.getElementById("audit_ghdwdsh");
		if(type=="0"){
