...
<script type="text/javascript">
	function onDelete(){
		...
		if(r){
			from.action="${ctx}/drugState/recoverycell/dlrdel.html";
			from.submit();
		}else{
			return;
		}
    ...
    function add(){
    	...
    	form.action="${ctx}/drugState/recoverycell/dlradd.html";
    	form.submit();
    }
	function deletess(a){
...
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;恢复销售管理&nbsp;>&nbsp;恢复销售录入</font>
	<div class="tab_biao">
...
<div class="tab_con">
	<div class="caozuo_box">
		<form action="${ctx}/drugState/recoverycell/dlrlist.html" method="post">
			<input type="hidden" name="ptmeth" value="${ptmeth}"/>
			<input type="hidden" name="tz" value="${thispage}"/>
			通用名称：<input name="ypname" type="text" value="${ypname}" class="text1"/>
			<shiro:hasPermission name="RecoverySale_recorded_find">
...
<form action="" id="recordsss">
	...
	<display:table name="reslist" cellspacing="0" cellpadding="0" requestURI=""  style="width:98%;" id="records" pagesize="${pageSize}" class="table" export="true" size="${resultSize}" partialList="true">
		...
		<shiro:hasPermission name="RecoverySale_recorded_modify">
		<img src="../../images/edit.gif"/><a href="${ctx}/drugState/recoverycell/dlredit.html?id=<c:out value="${records.id}"/>&thispage=${thispage}">
			<fmt:message key="button.update"/>
		</a>
		</shiro:hasPermission>
		<c:choose>
			<c:when test="${deleteItemFlag eq 'true'}">
				<shiro:hasPermission name="RecoverySale_recorded_delete">
...
<form id="delForm" method="post">
	...
	<shiro:hasPermission name="RecoverySale_recorded_add">
