...
<body>
	...
	<form action="comfrimRejectProcurement.html" name="edit_xsryybh" id="edit_xsryybh" method="post" >
        ...
        <table border="0" cellspacing="0" cellpadding="0" class="xx_table">
          ...
          <tr>
           ...
           <tr>
          	...
          	<td>
          		&nbsp;采购人员&nbsp;<input type="radio" name="personType" value="0" disabled='false' class="easyui-validatebox" data-options="required:true">
          		&nbsp;提货人员&nbsp;<input type="radio" name="personType" value="1" disabled='false' class="easyui-validatebox" data-options="required:true">
          	</td>
          </tr>
          <tr>
            <th valign="top">驳回原因：</th>
            <td colspan="3"><textarea name="reject_cause" cols="94" rows="4" class="textarea" readonly="readonly">${procurementStaff.reject_reason}</textarea></td>
          </tr>
          <tr height="80">
            <td colspan="4" align="center" valign="bottom">
           		 <input type="hidden" id="save_type" name="save_type" value=""/>
           		 <input type="hidden" name="id" value="${procurementStaff.id }">
                <input name="" type="button" class="btn_big" value="确认驳回" onclick="update_xsryybh()"/>
	...
	$(function(){
		...
		function update_xsryybh(){
		var con = confirm("是否确认驳回！");
		if(con){
		  $('#edit_xsryybh').form('submit',{  
			    success: function(data){  
				var json = jsonParse(data);
				if(json.success!=null && json.success!=""){
					alert(decodeURI( json.success));
				}
				location.href="procurementStaffList.html?type=3";
			    }  
			});  
		}else{
			return;
