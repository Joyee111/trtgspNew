...
<title>查看已审核首营企业</title>
</head>
<body>
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">企业资质管理&nbsp;>&nbsp;首营企业&nbsp;>&nbsp;查看已审核首营企业</font>
		  <%
       ...
       <!--  <tr>
        ...
        -->  
          ...
          <tr>
           ...
           <td>
           		<img src="../images/pdf.gif" />
            	<a href="../download.html?fileName=${waitEnter.licence_path}"><%=enterptise.getCompanyName()+"许可证"+StringUtil.fileExtensions(enterptise.getLicence_path())%></a>
           ...
           <%} %>
            <th>许可证到期日期：</th>
            <td>
            	<input type="text" name="licence_date" class="easyui-datebox"  size="40" value="${waitEnter.licence_date}" data-options="disabled:true"/>
            </td>
          </tr>
          <tr>
            <th>认证证书：</th>
           ...
           <%} %>
            <th>开户信息到期日期：</th>
            <td>
            	<input type="text" name="accountDate" class="easyui-datebox"  size="40" value="${waitEnter.accountDate}" data-options="disabled:true"/>
            </td>
          </tr>
          <tr>
            <th>年检证明：</th>
          ...
          <%--  <tr>
           ...
           <td>
           		<img src="../images/pdf.gif" />
            	<a href="../download.html?fileName=${waitEnter.business_licence_inspection_path}"><%=enterptise.getCompanyName()+"营业执照"+StringUtil.fileExtensions(enterptise.getBusiness_licence_inspection_path())%></a>
           </td>
           <%}else{ %>
            <td>
       ...
       <table border="0" cellspacing="0" cellpadding="0" class="xx_table2">
            ...
            <td colspan="4" align="center" valign="bottom">
           		...
           		<input type="hidden" id="" name="id" value="${waitEnter.id }"/>
           		<shiro:hasPermission name="FirstEnterprise_audited_return">
           		 <input name="" type="button" class="btn_big" value="确认驳回" onclick="comfrin_hggys()"/>
           		 </shiro:hasPermission>
        ...
        function comfrin_hggys(){
		  $('#audit_syqydsh').attr("action","confirmReject.html");
		  var comfrim_sure = confirm("是否确认驳回！");
		  if(comfrim_sure){
		  		  $('#audit_syqydsh').form('submit',{  
