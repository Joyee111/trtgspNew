...
public class CalculateUtil {
    ...
    public static String strCast(String str) {
        ...
        Pattern p=Pattern.compile("[\u4e00-\u9fa5]");//匹配中文
     ...
     * @return
     */
    public static String khys(String gs){
        String result = "";
        int startIndex;
        int endIndex;
        String tempGs = "";
        while(gs.lastIndexOf("(")!=-1){
            startIndex = gs.lastIndexOf("("); //先算最里面或是最后面的括号
            endIndex = gs.indexOf(")", startIndex);
            tempGs = gs.substring(startIndex+1, endIndex);
            result = szys(tempGs);
            gs = gs.replace("("+tempGs+")", result);
        }
        result = szys(gs);
        return result;
    }
