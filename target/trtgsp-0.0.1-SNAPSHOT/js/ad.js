...
function tabswitch(c, config){
	...
	for(var i = 0; i < this.tabs.length; i++){
		...
		try{
			...
			$i(this.tabs[i].id + '_title_1').onmouseover = function(){
				_this.pause = true;
			};
			...
			$i(this.tabs[i].id + '_title_1').onmouseout = function(){
				_this.pause = false;
			};
		}catch(e){}
	}
	...
	this.start = function(e, r, n){
		...
		if(r){
			curr_tab = $i(_this.container.id + '_' + rand(5));
		}else{
			if(n){
		...
		for(var i = 0; i < _this.tabs.length; i++){
			if(_this.tabs[i]==curr_tab){
				...
				try{
					...
					$i(_this.tabs[i].id + '_title_1').style.display = "block";
				}catch(e){}
			}else{
				...
				try{
					...
					$i(_this.tabs[i].id + '_title_1').style.display = "none";
				}catch(e){}
			}
		}
...
_jsc.util.addEvent(window, 'load', function() {
		if ($i('banners')) {	
			...
			setInterval("banners.start(null, null, 1);", 2000);
		}
