...
<body>
		<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">企业资质管理&nbsp;>&nbsp供货单位销售人员&nbsp;>&nbsp;供货单位销售人员审核</font>
		 <div class="tab_biao">
			...
			<shiro:hasPermission name="SalesQualify_auditing">
				<a href="salesStaff.html?type=1" class="on"><span><font>审核</font> </span>
				</a>
			</shiro:hasPermission>
			...
			<shiro:hasPermission name="SalesQualify_reject">
				<a href="salesStaff.html?type=3"><span><font>已驳回</font> </span>
				</a>
			</shiro:hasPermission>
		</div>
    	 ...
    	 <div class="tab_con">
        	...
        	<form name="query_sxrydlr" id="query_sxrydlr" action="querySalesStaff.html" method="post">
               	...
               	<shiro:hasPermission name="SalesQualify_auditing_find">
                <input name="" type="submit" class="btn_big" value="查询"/>
                </shiro:hasPermission>
                <shiro:hasPermission name="SalesQualify_auditing_yes">
                 <input name=""  type="button" class="btn_big" value="批量审批" onclick="bathcAuditSalesStaff(0)"/> 
                 </shiro:hasPermission>
                   <shiro:hasPermission name="SalesQualify_auditing_no">
                 <input name="" type="button" class="btn_big" value="批量驳回" onclick="bathcAuditSalesStaff(1)"/>
                 </shiro:hasPermission>
           </form>
            </div>
            <form name="batchAudit_xsry" action="batchAuditSalesStaff.html" id="batchAudit_xsry" method="post" >
 			<display:table name="salesStaffList" cellspacing="0" cellpadding="0" requestURI="" style="width:98%;" id="sale" pagesize="${pagesize}" class="table" export="true" size="${resultSize}" partialList="true">
				...
				<shiro:hasPermission name="SalesQualify_auditing_look">
				<display:column titleKey="操作" autolink="true" media="html"  class="border_rnone">
				<img src="../images/look.gif"/><a href="editSalesStaff.html?id=${sale.id}&type=1">查看</a>
				</display:column>
				</shiro:hasPermission>
			 ...
			 <input name="auditType" id="auditType" type="hidden"  value="" />
			</form>
	...
	function bathcAuditSalesStaff(str){
		//var from =document.getElementById("del_syqydlr");
		var r=confirm("是否确认操作！")
		$("#auditType").val(str);
		if(r){
			$("#batchAudit_xsry").form("submit",{
				 success: function(data){  
				var json = jsonParse(data);
				if(json.success!=null && json.success!=""){
					alert(decodeURI(json.success));
				}
				location.href="salesStaff.html?type=1";
			    }  
			});
		}else{
			return;
