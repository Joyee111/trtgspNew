...
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;退货验收管理&nbsp;>&nbsp;已复检</font>
<div id="syqy_tab" >
	<div class="tab_biao">
	<!--
		...
		<a href="yshlist.html" class="on" ><span><font>已复检</font>
		</span>
		</a>
		</shiro:hasPermission>
		<shiro:hasPermission name="ReturnCheckAccept_rollback">
		<a href="htlist.html" ><span><font>回退审核</font>
		</span>
		</a>
		</shiro:hasPermission>
	</div>
...
<div>
	...
	<div class="caozuo_box">
		<form action="${ctx}/drugState/checkreturn/yshlist.html"  method="post" >
			<input type="hidden" name="ptmeth" value="${ptmeth}"/>
			<input type="hidden" name="tz" value="${thispage}"/>
			复检日期：<input name="yhdate" type="text" value="${yhDate}"  class="easyui-datebox text1">
		...
		<form action="${ctx}/drugState/checkreturn/printCheckReturn.html" id="export_form" method="post" target="_blank">
				<input type="hidden" name="checkReturnId" value="0010527" id="checkReturnId"/>
				<input type="button" value="药品质量验收报告单" class="btn_big" onclick="exportExcel()"/>
		</form>
	</div>
</div>
<form action="" id="recordsss">
	...
	<display:table name="recordlist" cellspacing="0" cellpadding="0" requestURI=""  style="width:100%;" id="records" pagesize="${pageSize}" class="table" export="true" size="${resultSize}" partialList="true">
		...
		<display:column property="invalidQuantity" escapeXml="true" sortable="true" titleKey="退回数量" ></display:column>
		...
		<display:column property="reviewTime"  escapeXml="true" sortable="true" titleKey="复检日期" />
		<display:column property="visualExamination"  titleKey="复检结论"  ></display:column>
		
		<display:column titleKey="user.operate" autolink="true" media="html">
		<img src="../../images/look.gif"/>
		<a href="findCheckRetrunAcceptNotes.html?id=${records.id}">查看修改记录</a>
		<shiro:hasPermission name="ReturnCheckAccept_audited_look">
