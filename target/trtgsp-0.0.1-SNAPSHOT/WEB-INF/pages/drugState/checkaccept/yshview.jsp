...
<script type="text/javascript">
	function back(){
		...
		$("#checkConclusion2").val( $("#checkConclusion").val());
		$("#result2").val($("#result").val());
		$("#checkIsQualified2").val($("#checkIsQualified").val());
...
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;验收管理&nbsp;>&nbsp;查看已审核验收</font>
...
<body>
	...
	<th width="150">处理结果：</th>
	 <c:if test="${specialManegement=='0'}">
	   <td class="left" style="width:350px;">
		<input type="text" name="result" id="result" value="${mc.result2}" class="text1" size="40"/>
		</td>
	  </c:if>
	  <c:if test="${specialManegement=='1'}">
	   <td colspan="3">
		<textarea type="" name="result" id="result" cols="94" rows="4" class="textarea" >${mc.result2}</textarea>
	   </td>
	  </c:if>
	</tr>
	<tr>
	...
	<c:if test="${specialManegement=='0'}">
	 <td colspan="3">
		<textarea type="" name="checkConclusion" id="checkConclusion" cols="94" rows="4" class="textarea" >${mc.checkConclusion2}</textarea>
	</td>
	</c:if>
	<c:if test="${specialManegement=='1'}">
	 <td colspan="3">
...
<table id="condition_table" class="table_yh"  border="0" cellpadding="0" cellspacing="1" width="100%" align="center">
	<thead class='pn-lthead'>
	 <tr>
  	...
  	<table>
		<tr align="center">
			<shiro:hasPermission name="CheckAccept_audited_return">
			 	<td><input type="button" class="btn_big" value="确认" onclick="back()"/></td>
			 </shiro:hasPermission>	
 	...
 	<form action="backcheck.html" id="check_one">
 		...
 		<input type="hidden" id="specialManegement" name="specialManegement" value="${specialManegement}"/>
 		<input type="hidden" id="checkIsQualified2" name="checkIsQualified2" value=""/>
 		<input type="hidden" id="result2" name="result2" value=""/>
 		<input type="hidden" id="checkConclusion2" name="checkConclusion2" value=""/>
 	</form>
 	<script type="text/javascript">
