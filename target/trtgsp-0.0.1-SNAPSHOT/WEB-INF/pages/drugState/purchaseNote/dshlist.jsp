...
<script type="text/javascript">
	...
	function batchAuditPurchaseOrder() {
		var selectCheckBox = document.getElementsByName("ids");
		var flag = false;
		for ( var i = 0, len = selectCheckBox.length; i < len; i += 1) {
...
<div class="tab_con">
	<div class="caozuo_box">
		<form action="${ctx}/drugState/purchaseNote/dshlist.html" method="post" >
...
<div class="tab_con">
<form action="batchAuditPurchaseOrder.html" id="recordsss">
		<input type="hidden" name="ptmeth" value="${ptmeth}"/>
		<input type="hidden" name="tz" value="${thispage}"/>
	<div class="ceng_mar5">
	<display:table name="reslist" cellspacing="0" cellpadding="0" requestURI=""  style="width:100%;" id="records" pagesize="${pageSize}" class="table" export="true" size="${resultSize}" partialList="true">
		...
		<shiro:hasPermission name="PurchasingOrder_confirm_modify">
		<img src="../../images/edit.gif"/><a href="${ctx}/drugState/purchaseNote/viewPurchaseOrder.html?type=dsh&id=<c:out value="${records.id}"/>&thispage=${thispage}">
	...
	<!--屏蔽批量确认div class="">
		<c:if test="${dateDisableFlag eq 'true'}">
			<input type="button" class="btn_big" value="批量确认" onclick="batchAuditPurchaseOrder()"/>
		</c:if >
	</div-->
</form>
<form id="baseForm">
...
<form id="delForm">
	...
	<shiro:hasPermission name="PurchasingOrder_add">
	 <input type="button"  value="新增" class="btn_big" onclick="add()" />
	 </shiro:hasPermission>
	 <shiro:hasPermission name="PurchasingOrder_del">
