...
<script type="text/javascript">
	function pass(){
		...
		$("#counts").val($(".pn-ltbody tr").length);
		if(checkSubmit()){
			form.submit();
		}
	}
...
<font style="font-family:'宋体';font-weight:normal; font-size: 12;color:#8d3c01;padding: 10px;">药品状态管理&nbsp;>&nbsp;验收管理&nbsp;>&nbsp;查看批注验收</font>
	 ...
	 <tr>
	  ...
	  <c:forEach items="${chIt}" var="item" varStatus="index">
	  	<tbody class="pn-ltbody">
	  		<tr>
	  			<td ><input type="text" name="pingming${index.index }" value="${item.qualityMidicine.commonname }" readonly="readonly" class="text1" size="10" style="border: none;"/><input type="hidden" name="pingmingNumber${index.index }" value="${item.qualityMidicine.id }" readonly="readonly" class="text1" size="10" style="border: none;"/></td>
	  			...
	  			<td ><input type="text" name="guige${index.index }" value="${item.qualityMidicine.specifications }" readonly="readonly" class="text1"  size="10" style="border: none;"/></td>
	  			...
	  			<td ><input type="text" name="hegeshuliang${index.index }" id="hegeshuliang${index.index }" value="${item.qualifiedQuantity}"  class="text1"  size="8" style="border: none;" onchange="countQuantity('shuliang${index.index }','hegeshuliang${index.index }','buhegeshuliang${index.index }','hegeshuliang${index.index }')" /></td>
	  			<td ><input type="text" name="buhegeshuliang${index.index }" id ="buhegeshuliang${index.index }" value="${item.unqualifiedAmount}" class="text1"  size="8" style="border: none;" onchange="countQuantity('shuliang${index.index }','hegeshuliang${index.index }','buhegeshuliang${index.index }','buhegeshuliang${index.index }')" /></td>
	  			...
	  			<td>
	  			<input type="text" name="buhegexiang${index.index }" value="${item.unqualifiedItems}" class="text1"  size="10" style="border: none;"/>
	  			</td>
	  			<td>
					<select id="chuzhicuoshi${index.index }" name="chuzhicuoshi${index.index }" cssclass="text2">
  	...
  	<table>
		<tr align="center">
		  	<td><input type="button" class="btn_big" value="批注" onclick="pass()"/></td>
