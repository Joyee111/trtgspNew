...
<div class="box_big">
<form:form commandName="re" method="post" action="saveOrUpdata.html?method=add" id="res">
<form:hidden path="id"/>
	...
	<tr>
		...
		<c:choose>
				<c:when test="${dateDisableFlag == 'true'}">
					<input type="text" class="easyui-datebox" name="deliveryDate" id="deliveryDate" value="${re.deliveryDate}"  data-options="required:true,disabled:true"  size="40"/>
				</c:when>
...
<!-- 	<tr>
	...
	<th valign="top">退货原因：</th>
	 <td colspan="3">
		<textarea type="" name="returnReason" id="returnReason" cols="94" rows="4" class="textarea" >${re.returnReason}</textarea>
	</td>
	</tr>
</table>
...
function find(a){
	...
	$.post("${ctx}/drugState/returnRecords/findxshd.html",{
		...
		for(var s=0; s<jsons.length; s++)  {
		    	...
		    	//$("#deliveryDate").datebox('setValue',j.xiaoshouriqi);//退货时间不再设置为销售日期
				...
				"<td> " +
					...
					" <input type=\"text\" style=\"width:40px;\" value=\""+j.shuliang+"\"  name=\""+shuliang+"\" id=\""+shuliang+"\" onchange=\"compareOldAndNew('"+shuliang+"','"+oldshuliang+"')\"/> "+
				...
				"</td> " +
				"<td> " +
					" <input type=\"text\" style=\"width:100px;\" value=\""+j.shengchanriqi+"\" name=\""+shengchanriqi+"\" id=\""+shengchanriqi+"\" class='easyui-datebox text1' disabled='true' /> "+
				"</td> " +
				"<td> " +
...
var i=0;
    ...
    $.parser.parse($("#condition_table"));
    	...
    	if(typeof(method)!= "undefined" && method ==="add"){
    		$("#deliveryDate").datebox("setValue",getCurrentDate());
    	}
   ...
   function checkQuantity(){
	 	var oldObj = $('input[name^="oldshuliang"]');
	 	var tdObj = $('input[name^="shuliang"]');
	 	if(tdObj.size()!=oldObj.size()){
	 	 	return false;
	 	}
	 	for(var i = 0 ;i<tdObj.size();i++){
	 	 	if(parseFloat(tdObj[i].value) < 0){
				alert("保存数量不能为负数！");
				return false;
	 	 	}
	 	 	if(parseFloat(tdObj[i].value) > parseFloat(oldObj[i].value)){
	 	 		alert("保存数量不能大于原来数量！");
	 	 		return false;
	 		}
